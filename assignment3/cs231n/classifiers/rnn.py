...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # (3):
        h, step_cache = forward(x, h0, Wx, Wh, b)
        # (4):
        ...
        dout, grads['W_vocab'], grads['b_vocab'] =\
                                    temporal_affine_backward(dout , cache_score)
        dx, dh0, grads['Wx'], grads['Wh'], grads['b'] = \
                                    backward(dout, step_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        if self.cell_type == 'lstm':
            cell_state = np.zeros_like(prev_h)
        # Start token to 1st embedded word:
        word_emb, _ = word_embedding_forward(np.array([self._start]*N).T, W_embed)
        # make an step:
        step = 0
        while step < 30:
            if self.cell_type == 'rnn':
                h_next, _ = rnn_step_forward(word_emb, prev_h, Wx, Wh, b)
            else:
                h_next, cell_state, _ = \
                    lstm_step_forward(word_emb, prev_h, cell_state, Wx, Wh, b)
            score, _ = affine_forward(h_next, W_vocab, b_vocab)
            captions[:,step] = np.argmax(score, axis = 1)
            prev_h = h_next
            step += 1
        ############################################################################
        #                             END OF YOUR CODE                             #
